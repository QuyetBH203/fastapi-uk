...
async def create_module(module: CreateModuleDto, module_service: ModuleService = Depends(get_module_service)):
...
async def get_module_by_id(module_id: int, module_service: ModuleService = Depends(get_module_service)):
...
async def get_all_modules(page: int = 1, limit: int = 10, search: str | None = None, module_service: ModuleService = Depends(get_module_service)):
...
async def update_module(module_id: int, module: UpdateModuleDto, module_service: ModuleService = Depends(get_module_service)):
...
async def delete_module(module_id: int, module_service: ModuleService = Depends(get_module_service)):
